...
class AddUserAskView(View):
    # 用户添加咨询
    def post(self, request):    # 处理表单提交 ， 用post
        userask_form = UserAskForm(request.POST)
        if userask_form.is_valid():         # 判断该form是否有效
            user_ask = userask_form.save(commit=True)    # 当commit为True进行真正保存
            return HttpResponse('{"status": "success"}', content_type='application/json')  # 如果保存成功，返回json字符串，后面content_type是告诉浏览器的
        else:
            return HttpResponse('{"status": "fail", "msg": "添加出错"}', content_type='application/json')  # 如果保存失败，返回json字符串，并将form的报错信息通过msg传递到前端
